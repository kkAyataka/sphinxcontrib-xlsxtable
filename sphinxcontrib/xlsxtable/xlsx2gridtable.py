...
def get_use_indexes(
    min_index: int, max_index: int, includes_str: str, excludes_str: str
):
...
def gen_reST_grid_table_lines(
    ...
    if img_ws != None and img_ws.drawing != None:
        ...
        for anchor in img_ws.drawing.two_cell_anchors:
            ...
            table_cells[r][c].set_value(
                f".. image:: {file}.media/{os.path.basename(anchor.image_path)}"
            )
            ...
            for c, cell in enumerate(cols):
                ...
                if len(cell.values) > l:
                    ...
                    line_str += get_padding(
                        get_string_width(cell.values[l]), cell.width
                    )
                    line_str += " "
                else:
...
def main():
    ...
    p.add_argument(
        "--include-rows", type=str, default=None, help="Specify included rows"
    )
    p.add_argument(
        "--exclude-rows", type=str, default=None, help="Specify excluded rows"
    )
    p.add_argument(
        "--include-columns", type=str, default=None, help="Specify included columns"
    )
    p.add_argument(
        "--exclude-columns", type=str, default=None, help="Specify excluded columns"
    )
    p.add_argument("file", type=str, help="Target Excel file path")

    args = p.parse_args()

    draw_reST_grid_table(
        args.file,
        args.file,
        args.header_rows,
        args.sheet,
